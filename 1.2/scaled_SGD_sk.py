...
def model_creator(data_full, ticker, model_creator_starttime, cnt):
    ...
    #split the data
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=.2, random_state=23, shuffle=False)
    ...
    #select a model
    model = SGDRegressor(loss='huber', random_state = 23)
    ...
    model = RandomizedSearchCV(estimator=model, param_distributions = parameters,
        refit=True, cv = 5, n_iter = 25, verbose = 2, n_jobs=-1, random_state=23)

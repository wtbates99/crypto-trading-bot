...
def trade_placer(trade_data):
    ...
    ############
    trade_data_fixed = trade_data_fixed[trade_data_fixed['Tomorrows Predicted Close'] > trade_data_fixed['Todays Actual Close']]

    ###show stocks that passed through the requirement
    print('\n--------------------------------------------\nTrades that Passed\n--------------------------------------------\n', trade_data_fixed)

    # Get our account information.
    k = krakenex.API(API_KEY, SECRET_KEY)
    # Get the account balances
    response = k.query_private('TradeBalance', {'asset': 'ZUSD'})
    cash_balance = response['result']['tb']
    print(f'Your current cash balance is {cash_balance} USD.')

    cash_per_trade = round(float(cash_balance) / 2, 2)        
    cash_per_stock = round(cash_per_trade / len(trade_data_fixed), 2)
    ...
    for stocks in range(len(trade_data_fixed)):
        ...
        quantity = cash_per_stock / buy_price
        ...
        # Get the order ID for the buy order
        order_id = buy_order['result']['txid'][0]

        # Poll the order status until it is filled or cancelled
        while True:
            time.sleep(5)
            order_status = k.query_private('QueryOrders', {'txid': order_id})

            if order_status['result'][order_id]['status'] == 'filled':
                break
            elif order_status['result'][order_id]['status'] == 'canceled':
                raise Exception('Order was canceled')

        # Once the order is filled, place a stop loss and take profit order
        stop_loss_order = k.query_private('AddOrder', {
            'pair': pair,
            'type': 'sell',
            'ordertype': 'stop-loss',
            ...
            'close[price]': take_profit_price
        })

        # Get the order ID for the stop loss order
        stop_loss_id = stop_loss_order['result']['txid'][0]

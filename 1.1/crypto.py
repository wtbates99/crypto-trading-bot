...
def moredata(data_alter):
    ...
    data_alter["BTC_c_two"] = close_one
    data_alter["BTC_c_three"] = close_one
    data_alter["BTC_c_four"] = close_one
    data_alter["BTC_c_five"] = close_one
    data_alter["BTC_v_one"] = volume_one
    data_alter["BTC_v_two"] = volume_one
    data_alter["BTC_v_three"] = volume_one
    data_alter["BTC_v_four"] = volume_one
    data_alter["BTC_v_five"] = volume_one
    ...
    def additional_data(data, tick, close_name, volume_name):
        ...
        c_one = data.rolling(one_period).mean()[close_name] 
        c_two = data.rolling(two_period).mean()[close_name]
        c_three = data.rolling(three_period).mean()[close_name]
        c_four = data.rolling(four_period).mean()[close_name]
        c_five = data.rolling(five_period).mean()[close_name]
        v_one = data.rolling(one_period).mean()[volume_name] 
        v_two = data.rolling(two_period).mean()[volume_name]
        v_three = data.rolling(three_period).mean()[volume_name]
        v_four = data.rolling(four_period).mean()[volume_name]
        v_five = data.rolling(five_period).mean()[volume_name]
...
def model_creator(data_full, ticker, model_creator_starttime, cnt):
   ...
   #parameters to be tuned
    ...
    model = RandomizedSearchCV(estimator=model, param_distributions = parameters,
                                   refit=True, cv = 10, n_iter = 1000, verbose = 1, n_jobs=-1, random_state=23)
                                       #parameters to be tuned

...
def model_creator(data_full, ticker, model_creator_starttime, cnt):
   ...
   #parameters to be tuned
    ...
    model = RandomizedSearchCV(estimator=model, param_distributions = parameters,
                                   refit=True, cv = 5, n_iter = 25, verbose = 1, n_jobs=-1, random_state=23)
                                       #parameters to be tuned
    ...
    with open('model_' + ticker + '.pkl', 'wb') as f:
        pickle.dump(model,f)
    print("----------------/n Model for: ", ticker, "has been saved to file location\n----------------")
...
def trade_placer(trade_data):
    ...
    cash_for_trades = float(account.cash) * .71
    cash_per_stock = cash_for_trades * 0.20
